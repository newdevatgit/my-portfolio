...
const projects = [
  ...
  {
      title:"Furniro webpage",
      description:"Furniro is an eccomerse site for various types of furniture. it is made by react and tailwind css. mongo db and node js for backend.",
      imageUrl:"/Furniro-home.png",
      tags:["React","Tailwind css","Mongo DB","Node js" ],
      githubUrl:"https://github.com/newdevatgit/furniro-frontend",
      liveUrl:"https://furniroweb.netlify.app/",
  },
    {
      title: "Helpdesk App",
...
const ProjectCard = ({ title, description, imageUrl, tags, githubUrl, liveUrl }) => (
    <div 
      className="bg-gray-200 p-4 rounded-2xl shadow-xl overflow-hidden transform hover:-translate-y-1 transition-all duration-300 hover:shadow-2xl"
    >
        <img 
          src={imageUrl} 
          alt={title} 
        ...
        />
        <div className="p-6">
            <h3 className="text-xl font-bold text-black dark:text-gray-900 mb-2">{title}</h3>
            <p className="text-gray-700 text-sm dark:text-gray-600 mb-4">{description}</p>
            <div className="flex flex-wrap gap-2 mb-6">
                {tags.map(tag => (
                    <span key={tag} className="bg-black text-gray-400 text-xs font-semibold px-2.5 py-1 rounded-full">{tag}</span>
                ))}
            </div>
            <div className="flex gap-4">
                <a 
                    ...
                    className="inline-flex items-center gap-2 w-full justify-center 
                               bg-gray-100 hover:bg-gray-200 text-gray-700 dark:bg-gray-800 dark:hover:bg-gray-900 dark:text-gray-300
                               font-semibold px-2 py-2 rounded-lg transition-colors shadow-sm"
                >
                ...
                <a 
                    ...
                    rel="noopener noreferrer" 
                    className="inline-flex items-center gap-2 w-full justify-center 
                               bg-indigo-500 hover:bg-indigo-600 text-white dark:bg-indigo-700 dark:hover:bg-indigo-800
                               font-semibold px-4 py-2 rounded-lg transition-colors shadow-md"
                >
  ...
  return (
    <section id="projects" className="py-10 bg-gray-300 lg:p-6 text-gray-800">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="grid md:grid-cols-2 font-reading lg:grid-cols-3 gap-6">
